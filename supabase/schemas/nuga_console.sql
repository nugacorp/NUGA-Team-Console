-- NUGA Console-owned data only. Hermes remains canonical for boards, tasks,
...
create schema if not exists nuga_console;
...
create table nuga_console.decisions (
  id uuid primary key default gen_random_uuid(),
...
create table nuga_console.deliverables (
  id uuid primary key default gen_random_uuid(),
  code text not null unique check (length(code) between 1 and 64),
  title text not null check (length(title) between 1 and 240),
...
create table nuga_console.audit_events (
  id uuid primary key default gen_random_uuid(),
...
create index task_extensions_deadline_idx on nuga_console.task_extensions (deadline) where deadline is not null;
create index decisions_status_deadline_idx on nuga_console.decisions (status, deadline);
create index decisions_hermes_task_idx on nuga_console.decisions (hermes_board_slug, hermes_task_id) where hermes_task_id is not null;
create index deliverables_hermes_task_idx on nuga_console.deliverables (hermes_board_slug, hermes_task_id) where hermes_task_id is not null;
create index audit_events_occurred_at_idx on nuga_console.audit_events (occurred_at desc);
...
alter table nuga_console.deliverables enable row level security;
alter table nuga_console.deliverables force row level security;
alter table nuga_console.audit_events enable row level security;
alter table nuga_console.audit_events force row level security;

revoke all on all tables in schema nuga_console from public, anon, authenticated;
revoke all on all sequences in schema nuga_console from public, anon, authenticated;
